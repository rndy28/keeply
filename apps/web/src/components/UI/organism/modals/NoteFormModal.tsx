import { IconArchive, IconCalendarTime, IconPalette, IconPencil, IconPinned, IconX } from "@tabler/icons";
...
const IconContainer = styled.div<{ isActive?: boolean }>`
  ...
  cursor: pointer;
  background-color: ${(p) => (p.isActive ? (p.theme.state === "light" ? "#D8DEE9" : "#4C566A") : "transparent")};
  &:hover {
...
const NoteFormModal = (data: NoteWithLabelsIncluded) => {
  ...
  const onGetReminderTime = (data: { date: Date; time: string }) => {
    const dateNumber = stringDateToNumber(`${formatDate(data.date, undefined, { dateStyle: "short" })} ${data.time}`);
    ...
    <Modal>
      <Container variants={formVariant} initial="hidden" animate="visible" exit="exit">
                ...
                {note.labels.map((label, idx) => (
                  ...
                  >
                    ...
                    {labelName === label.name && (
                      <IconX size={15} css="position: relative; bottom: 2px" onClick={onRemoveLabel(label.id)} />
                    )}
                  </Label>
                ))}
              </Flex>
        ...
        <AnimatePresence>
          ...
          {open === "label" && <LabelForm onIncluded={onIncludeLabel} includedLabels={note.labels} />}
          {open === "time" && <DateTimePicker onSuccessSubmit={onGetReminderTime} selectedTime={note.time} />}
        </AnimatePresence>
      </Container>
    </Modal>
  );
