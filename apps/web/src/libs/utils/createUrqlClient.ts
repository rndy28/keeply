      ...
      updates: {
        Mutation: {
          ...
          signin: (_result, _args, cache, _info) => {
            betterUpdateQuery<SigninMutation, MeQuery>(cache, { query: MeDocument }, _result, (result, query) => {
          ...
          signup: (_result, _args, cache, _info) => {
            betterUpdateQuery<SignupMutation, MeQuery>(cache, { query: MeDocument }, _result, (result, query) => {
          ...
          deleteLabel: (_result, args, cache, _info) => {
            betterUpdateQuery<DeleteLabelMutation, LabelsQuery>(
              ...
              (_, query) => {
                return {
                  labels: query.labels.filter((label) => +label.id !== (args as DeleteLabelMutationVariables).labelId),
                };
              },
            );
            invalidateQuery(cache, "notes");
          ...
          updateLabel: (_result, _args, cache, _info) => {
            betterUpdateQuery<EditLabelMutation, LabelsQuery>(
              ...
              (result, query) => {
                return {
                  labels: query.labels.map((label) => (label.id === result.editLabel.id ? result.editLabel : label)),
                };
              },
            );
            invalidateQuery(cache, "notes");
          ...
          deleteNoteForever: (_result, args, cache, _info) => {
            betterUpdateQuery<DeleteNoteForeverMutation, NotesQuery>(
              ...
              (_, query) => {
                return {
                  notes: query.notes.filter((note) => +note.id !== (args as DeleteNoteForeverMutationVariables).noteId),
                };
              },
            );
          },
          clearTrash: (_result, _args, cache, _info) => {
            betterUpdateQuery<ClearTrashMutation, NotesQuery>(cache, { query: NotesDocument }, _result, (_, _query) => {
              return {
                notes: [],
              };
            });
